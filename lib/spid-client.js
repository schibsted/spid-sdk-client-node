...
function _getSPiDUserObject(user) {
    return {
        id: user.userId,
        displayName: user.displayName || 'Anonymous',
        formattedName: user.name.formatted || 'Anonymous',
        email: user.email,
        photo: user.photo,
...
class SpidClient {
            ...
            if (shouldValidate || !ctx.session.user) {
                if (ctx.session.spidData &&
                    ...
                    try {
                        ...
                        ctx.session.user = yield _getSPiDUserObject(userData.data);
    ...
    requireAuthenticatedUser(shouldRedirect) {
        ...
        return function *spidRequireAuthenticatedUser(next) {
            ...
            } else if (_.isString(ctx.params.id) && _.isObject(ctx.session.user.roles)) {
                if (!_.result(_.find(ctx.session.user.roles, { id: ctx.params.id }), 'id')) {
                    ctx.throw('Unauthorized', 401);
                }
            }
    ...
    /**
     * Returns user data for a given field and value
     */
    *getUser(field, user) {
        const spid = this;
        let oauthResponse;
        ...
        try {
            let queryString = {
                'oauth_token': oauthResponse.access_token,
                'fields': 'name,displayName,userId,email,photo'
            };
            queryString[field] = user;

            let spidUser = yield _spidRequest(spid, {
                method: 'GET',
                uri: spidEndpoints.users,
                qs: queryString
            });

            return _getSPiDUserObject(spidUser.data[0]);
        } catch (e) {
            console.log('[SpidClient] Could not find ' + field + ' ' + user);
        }
    }
}
